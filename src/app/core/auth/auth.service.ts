...
export class AuthService {
  ...
  login(email: string, password: string) {
    return this.http.post<AuthResponse>(`${this.API_URL}/login`, { email, password }).pipe(
      tap((res) => {
        // El backend devuelve solo el token; puedes ajustar esta lógica según la respuesta real.
        this.setAuth(res.token, { email } as any);
  ...
  // Nuevo logout: primero llama al backend, después limpia FE
  logout(): void {
